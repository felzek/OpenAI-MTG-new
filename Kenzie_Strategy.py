...
probability_array = np.full((n,n,n), 0.5)
count_array = np.full((n,n,n), 0)
decision_array = np.full((n,n,n), 0.5)
for x in range(n):
    for y in range(n):
        for z in range(n):
            decision_array[x,y,z] = int(math.ceil(x/2))
                
#defining decision function
def decision(count_array, self_creatures=0, opponent_creatures=0, self_life=20, opponent_life=20):
    temp_index = abs(self_creatures - opponent_creatures)
    if self_creatures > opponent_creatures:
        attacking_creatures = decision_array[temp_index, self_life, opponent_life] + opponent_creatures
        count_array[temp_index, self_life, opponent_life] += 1
        count_array = count_array.astype(int)
        return int(attacking_creatures)
    # modify this else statement somehow
    else:
        x = np.random.random()
        attacking_creatures = self_creatures
        if x > 0.5:
            return int(attacking_creatures)
        else:
            return 0
